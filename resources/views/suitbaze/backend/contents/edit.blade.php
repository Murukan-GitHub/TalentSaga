...
@section('content')
    <nav>
        ...
        <ul class="breadcrumb">
            ...
            <li><a href="{{ route('backend.content.show', ['id'=>$content->id]) }}">{{ $content->getFormattedValue() }}</a></li>
            <li>Update</li>
        </ul>
    </nav>
            ...
            {!! Form::model($content, ['files'=> true, 'id'=>'content_form']) !!}
                ...
                <div class="bzg">
                    <div class="bzg_c" data-col="l9" data-offset="l3">
                        <a class="btn btn--red" onClick="return confirm('Your changes will be not saved! Are you sure?');" href="{{ route('backend.content.show',['id'=>$content->id]) }}">Cancel</a>
                        &nbsp;
