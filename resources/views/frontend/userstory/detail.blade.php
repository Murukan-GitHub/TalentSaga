        ...
        <div class="container">
            <article class="success-story-article">
                <header class="success-story-article-header">
                    <a class="success-story-article-header-back" href="{{ route('user.story.list') }}">
                        <span class="fa fa-fw fa-chevron-left"></span> Back
                    </a>
                    <img class="success-story-article-header-img" src="{{ ($userStory->cover_image ? $userStory->cover_image_large_cover : asset('frontend/assets/img/success-story-article-header.jpg')) }}" alt="">

                    <div class="success-story-article-header-info">
                        <h2 data-prefix="Success stories">{{ $userStory->title }}</h2>
                        <time>{{ $userStory->created_at->format(app()->getLocale() == 'de' ? 'd.m.Y' : 'd F Y') }}</time>
                    </div>
                </header>

                {!! htmlspecialchars_decode($userStory->content) !!}
