...
use Input;
use Route;
use App\Repositories\UserProfileExpertiseRepository;
use App\Models\UserProfileExpertise;

class UserExpertiseController extends BackendController
{
    /**
     * Override Default Constructor.
     *
     * @param UserProfileExpertiseRepository $_baseRepo
     * @param UserProfileExpertise           $_baseModel
     */
    public function __construct(UserProfileExpertiseRepository $_baseRepo, UserProfileExpertise $_baseModel)
    {
        parent::__construct($_baseRepo, $_baseModel, 'user');
        $this->routeBaseName = 'backend.userexpertise';
        $this->viewBaseClosure = 'backend.userexpertises';
    ...
    public function postIndexJson() {
        ...
        // Filter Parameter
        $specificFilter = [];
        if (isset($param['user_profile_id'])) $specificFilter['user_profile_id'] = $param['user_profile_id'];
        // Return
        $renderedMenu = "";
    ...
    public function getCreate()
    {
        ...
        $userId = Input::get('user_profile_id');
        if ($userId && $userId > 0) {
            $baseObj->user_profile_id = $userId;
