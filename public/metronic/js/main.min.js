...
;(function ( window, document, $, undefined ) {
    ...
    var Site = {
        ...
        inputDashboardRangeDate: function () {
            var inputStart = document.querySelector('[dashboard-date-range-start]');
            var inputEnd = document.querySelector('[dashboard-date-range-end]');

            if ( !inputStart || !inputEnd) return;

            var inputStartFormId = inputStart.getAttribute('dashboard-date-range-start');
            var inputEndFormId = inputEnd.getAttribute('dashboard-date-range-end');
            ...
            var startElmt = rome(inputStart, startOpts);
            var endElmt = rome(inputEnd, endOpts);

            // startElmt.on('data', function(newDate) {
            //     $('#' + inputStartFormId).submit();
            // });

            // endElmt.on('data', function(newDate) {
            //     $('#' + inputEndFormId).submit();
            // });
        },
        ...
        datatableYadcf: function () {
            ...
            $.each($tables, function (index, table) {
                ...
                var $actions = $(tableID).closest('.table-manages').find('.table-actions');
                $(tableID).on('change', 'tbody tr .checkboxes', function () {
                    var datatableNbChecked = $(tableID).attr('data-datatable-yadcf-nb-checked') || 0;
                    if (datatableNbChecked == 0) {
                ...
                $('button.yadcf-filter-reset-button').addClass('btn btn-sm green btn-outline active').wrap("<span style=\"position:absolute;margin:3px 8px;top:0px;right:-90px;float:left;\"></span>");
        ...
        chart: function () {
            ...
            function generateChart( elem, type, opts ) {
                ...
                var currentOpts = $.extend(true, SuitmediaHelper.chartOpts[type], {});
        ...
        changeTableData: function () {
            ...
            $select.on('change', function() {
                ...
                SuitmediaHelper.changeTableData( dataURL, $target );
            });
        },
        ...
        todoList: function () {
            ...
            function createTodoLabel( label, inputElem, inputBtn ) {
                ...
                var UID = SuitmediaHelper.getUID();
        ...
        inputBrowseFiles: function () {
            ...
            var showAttachmentList = function (e) {
                ...
                if ( !SuitmediaHelper.supportFileAPI() ) return;
            ...
            function createAttachedList( files ) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if ( SuitmediaHelper.isImage( current.name ) ) {
                        result += 'fa-file-image-o';
                    } else if ( SuitmediaHelper.isPDF( current.name ) ) {
                        result += 'fa-file-pdf-o';
                    } else if ( SuitmediaHelper.isDOC( current.name ) ) {
                        result += 'fa-file-word-o';
                    } else if ( SuitmediaHelper.isPPT( current.name ) ) {
                        result += 'fa-file-powerpoint-o';
                    } else if ( SuitmediaHelper.isXLS( current.name ) ) {
        ...
        executionDocList: function () {
            ...
            var createNewExecDoc = function (e) {
                ...
                var selectCheckboxID    = SuitmediaHelper.getUID();
